...
class NativeSelfPlay:
    ...
    def __init__(self, policy_net, value_net, device: str = "cpu", *,
                 sims: int = 100, c_puct: float = 1.5,
                 dirichlet_alpha: float = _DIRICHLET_ALPHA, noise_frac: float = _NOISE_FRAC,
                 temperature: float = 1.0, temp_moves: int = 30, max_moves: int = 200):
        if not HAS_NATIVE:
            raise RuntimeError(
        ...
        self._engine = _native.SelfPlayEngine(
            ...
            noise_frac=noise_frac, temperature=temperature,
            temp_moves=temp_moves, max_moves=max_moves)
