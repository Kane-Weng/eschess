...
def train_policy_soft(net: PolicyNet, dataset: SelfPlayDataset, epochs: int,
...
def train_value(net: ValueNet, dataset: SelfPlayDataset, epochs: int,
                batch_size: int, lr: float, device: str) -> float:
...
def evaluate_gate(candidate: tuple[PolicyNet, ValueNet], best: tuple[PolicyNet, ValueNet],
                  games: int, sims: int, c_puct: float, max_moves: int,
                  device: str) -> float:
...
def run_rl(args: argparse.Namespace) -> None:
    ...
    for generation in range(1, args.generations + 1):
        ...
        print(f"gen {generation}: {len(gen_samples)} new samples, "
              f"{len(train_samples)} in buffer", flush=True)

        candidate_policy = copy.deepcopy(best_policy)
        candidate_value  = copy.deepcopy(best_value)
        policy_loss = train_policy_soft(candidate_policy, dataset, args.epochs,
                                        args.batch_size, args.lr, device)
        value_loss  = train_value(candidate_value, dataset, args.epochs,
                                  args.batch_size, args.lr, device)

        if args.gate_games > 0:
            gate_score = evaluate_gate((candidate_policy, candidate_value),
                                       (best_policy, best_value), args.gate_games,
                                       args.sims, args.c_puct, args.max_moves, device)
            accepted = gate_score >= args.gate_threshold
            print(f"gen {generation}: gate {gate_score:.3f} "
        ...
        with open(csv_path, "a", newline="") as fh:
            csv.writer(fh).writerow(
                [generation, len(train_samples), f"{policy_loss:.4f}",
                 f"{value_loss:.4f}", f"{gate_score:.4f}", int(accepted)])
...
def main() -> None:
    parser = argparse.ArgumentParser(description="AlphaZero-style self-play RL for the eschess networks.")
    parser.add_argument("--generations", type=int, default=5, help="self-play / train / gate cycles")
    parser.add_argument("--games-per-gen", type=int, default=20, help="self-play games each generation")
    ...
    parser.add_argument("--temperature", type=float, default=1.0, help="early-move sampling temperature")
    parser.add_argument("--temp-moves", type=int, default=30, help="plies sampled before going greedy")
    parser.add_argument("--max-moves", type=int, default=200, help="ply cap before a draw is adjudicated")
    parser.add_argument("--buffer-gens", type=int, default=3, help="generations of games kept for training")
    parser.add_argument("--gate-games", type=int, default=10, help="candidate-vs-best games (0 disables gating)")
    parser.add_argument("--gate-threshold", type=float, default=0.55, help="min candidate score to promote")
    parser.add_argument("--init-policy", default=None, help="warm-start policy weights (default: latest supervised)")
